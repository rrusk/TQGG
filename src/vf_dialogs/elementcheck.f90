...
!     - define options
      ...
      fullcolour=.true.
      if(TrHiOff) then
        call PanelHit(4, 5, 1, 'NONE        ', 12)
        elseIF (fullcolour) THEN
        call PanelHit(4, 5, 1, 'FULL COLOR  ', 12)
      ELSE
        call PanelHit(4, 5, 1, 'COLOR MARKER', 12)
      ENDIF
!      call PanelHit(4, 6, 2, 'SET CRITERIA', 12)

!     - following options not active till SET CRITERIA selected
...
! this code needs to be changed to implement a three item selector
!  with FULL COLOR, COLOR MARKER, NONE as the options.
        call PigSetTextColour(HitColor)
        if(TrHiOff) then
!              - change from none to full color
           ...
           fullcolour = .true.
           call PanelHit(4, 5, 1, 'FULL COLOR  ', 12)
        else if (fullcolour) then
!              - change from full color to color marker
           fullcolour = .false.
           call PanelHit(4, 5, 1, 'COLOR MARKER', 12)
        else
!              - change from color marker to none
           fullcolour = .false.
           TrHiOff = .true.
           call PanelHit(4, 5, 1,'NONE        ', 12)
        endif
...
!         - SET CRITERIA - make criteria options available
        if(.not.TrHiOff) then
          call Nup_FCriteria_Init (change)
        else
          call PigPutMessage('Cannot set criteria for NONE option')
        endif
      ELSEIF (hitnum .EQ. 3) THEN
!         - TOP: DISPLAY
! Changed from TopDisplay to Salpo for the PIGS menu system. (AM) June 93.
!       call Salpo('Display', Nrec)

      ELSEIF (hitnum .EQ. 4) THEN
!         - TOP: INFO
! Changed from TopInfo to Salpo for the PIGS menu system. (AM) June 93.
!       call Salpo('Info', Nrec)
!            call FlagTriScreen
...
!---------------------------------------------------------------------------*
      SUBROUTINE Nup_FCriteria_Init (change)
...
!     make options from FlagsTriangles unavailable
      call PigSetTextColour( NoHitColor )
      call DisablePanelHit(1)
      call DisablePanelHit(2)
!     - make criteria options available
      do i=11,17
        call ReenablePanelHit(i)
      end do
      return


      entry Nup_FCriteria_Ehandler (change, hitnum)
...
! Given:  an index to a triangle
! Returns:the code of the triangle
